...
class Luckey36 extends CI_Controller {

    
    public function cron_old2005()
{
    ...
    $already = $this->db->get_where('tbl_lucky36_results', [
    'period_id' => $period_id,
    'game_id'   => $gameId
])->row();
...
if ($already) {
    ...
    if ($manual_set == 1) {
        ...
        $bets = $this->db->get_where('tbl_lucky36_bet', [
            'period_id' => $period_id,
            'game_id'   => $gameId,
            'status'    => 'pending'
        ])->result();
        ...
        if (empty($bets)) {
            ...
            foreach ($bets as $bet) {
                ...
                $dealerCommission      = round($amount * 0.02, 2);
                $distributorCommission = round($amount * 0.005, 2);
    ...
    // INSERT RESULT
    // =========================
    $this->db->insert('tbl_lucky36_results', [
        'period_id' => $period_id,
        'game_id'   => $gameId,
    ...
    foreach ($bets as $bet) {
        ...
        if ($win_amount > 0) {
            ...
            // ✅ FIXED: Safe wallet update (no overwrite)
...
/*public function cron()
{
    ...
    foreach ($bets as $bet) {
        ...
        if ($win_amount > 0) {
            ...
            $this->db->where('id', $bet->id);
            $this->db->update('tbl_lucky36_bet', ['win_amount' => $win_amount]);
            
            // =============================================
            // CREDIT COMMISSIONS TO DEALER & DISTRIBUTOR
            // =============================================
            // Get user to find dealer and distributor
            $user = $this->db->get_where('tbl_users', ['id' => $bet->userid])->row();
            
            if($user && $user->dealer_id) {
                // Get dealer commission from bet
                ...
                if($dealer_commission > 0) {
                    ...
                    // Record dealer transaction
                    $this->db->insert('tbl_dealer_transactions', [
            ...
            if($user && $user->distributor_id) {
                // Get distributor commission from bet
                ...
                if($distributor_commission > 0) {
                    ...
                    $this->db->insert('tbl_distributor_transactions', [
...
public function place_bet()
{
    ...
    // =============================================
    // COMMISSION CALCULATION FOR BOTH
    // =============================================
    ...
    // Calculate Distributor Commission (0.5%)
    if($distributor_id) {
...
$winning_wallet = floatval(
    ...
    public function wallet_balance()
    {
        ...
        if (!$userid) {
            ...
            if (!$user) {
                ...
                $pending_amount = floatval($this->db_model->sum('amount', 'tbl_lucky36_bet', ['userid' => $userid, 'status' => 'pending']));
                $response = array(
                    'status'          => 'success',
    ...
    public function my_bets()
    {
        ...
        if ($chek_user <= 0) {
            ...
            $win_bets = $this->db_model->sum('amount', 'tbl_lucky36_bet', ['period_id' => $period_id - 1, 'status' => 'won', 'userid' => $userid]) + 0;
    ...
    public function show_result()
    {
        $gameId    = $this->get_game_id();
        $period_id = $this->db_model->select('period_id', 'tbl_games', array('id' => $gameId));
        $result    = $this->db_model->count_all('tbl_lucky36_results', array('period_id' => $period_id, 'game_id' => $gameId));
        $result2   = $this->db_model->select('win_number', 'tbl_games', array('id' => $gameId));
        $limit     = 10;
        $this->db->select('win_number');
        $this->db->from('tbl_lucky36_results');
        ...
        if ($result > 0) {
            $number = $this->db_model->select('win_number', 'tbl_lucky36_results', array('period_id' => $period_id));
            $response = array(
                'status'     => 'success',
                'number'     => $number,
                'last_res'   => $lstresult,
    ...
    public function win_bets()
    {
        ...
        if($chek_user <= 0){
            ...
            $win_bet  = $this->db->select_sum('amount')->where("DATE(date) =", $today_date)->where('userid', $userid)->where('status', 'won')->where('game_id', $this->get_game_id())->get('tbl_lucky36_bet')->row()->amount + 0;
    ...
    public function cancel_last_bet()
    {
        ...
        $this->db->order_by('id', 'DESC');
        $this->db->limit(1);
        $bet = $this->db->get('tbl_lucky36_bet')->row();

        if (!$bet) {
            $response = ['status' => 'error', 'message' => 'No pending bet found.'];
        } else {
            $this->db->where('id', $bet->id);
            $this->db->update('tbl_lucky36_bet', ['status' => 'cancelled']);
    ...
    public function repeat_last_bet()
{
    ...
    $this->db->insert('tbl_lucky36_bet', $insertData);
